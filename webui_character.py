...
DEFAULT_PARAMS = dict(gain=0.65, steps=15, hab=0.7, lo=0.10, hi=0.40,
                      ...
                      # 默认值在真实 vocal 轨迹上标定过(tune_gate_on_trace.py):
                      # 真实 vocal 的 p50≈2.3, 所以 θ0 取略高于它 —— 平时在阈值下方,
                      # 只有需求把 vocal 顶上去才开口。实测平静 21s/CV0.65, 应激 13s/CV0.66。
                      ada=1.0,          # 1 = 阈值适应(生物); 0 = 固定阈值(消融对照)
                      theta0=2.50,      # 基础阈值 θ₀ = 静息兴奋性, 不指定任何速率
                      k_adapt=0.60,     # 开口后阈值抬升 ∝ sqrt(超出量)
                      tau_adapt=35.0,   # 阈值回落时间常数(秒)
