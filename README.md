...
| 参数 | 脑增益、**脑步数每 tick**（时间尺度旋钮）、习惯化强度、感觉带上下限、四个驱动倍率、**发声门控（θ₀ 基础阈值 / k_adapt 适应抬升 / τ 适应回落 / 适应抖动 / 下行神经元→发声耦合）**、**自适应开关** |
| 图 1 | 内驱力曲线（饥饿/口渴/热冷/无聊/孤独/威胁） |
| 图 2 | **想说话冲动 u 与阈值 θ**，越过即开口 |
| 图 3 | 识别细胞读出（MN9 / DNp01 / DNp20 / DNp09） |
| 事件流 | 行为与台词，每条带 `连接组` / `回退` 标记 |

**发声门控没有任何"目标频率"**：阈值只有"开口时抬升 ∝√超出量、其余时间向 θ₀ 回落"两个动作，
频率是驱动/噪声/适应相互作用的结果。实测间隔 CV = 0.54（旧的伺服机制是 0.00，即节拍器）。
「自适应」开关是刻意的教学开关：关掉它就退回固定阈值，能直接看到病态。
